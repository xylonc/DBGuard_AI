...
def render_action_matches_template(action: dict, param: str, value: str | None = None) -> bool:
    """Check if render_action output matches the template for the same inputs.

    The template is set_config_parameter.sql.j2 which renders:
    ALTER SYSTEM SET {{ param_name | ident }} = '{{ param_value }}';

    For set_config: value must be provided and compared.
    For reset_config: no value comparison needed.

    Args:
        action: The typed action.
        param: The parameter name.
        value: The value for set_config (optional for reset_config).

    Returns:
        True if render_action output matches template output.

    Raises:
        ValueError: If template can't be rendered standalone (requires template_service.env).
    """
    from app.services.template_service import env

    if action.get("action") == "set_config":
        if value is None:
            raise ValueError("Value required for set_config comparison")
        ...
        return template_sql == rendered
    elif action.get("action") == "reset_config":
        template_str = "ALTER SYSTEM RESET {{ param_name | ident }}; SELECT pg_reload_conf();"
        template = env.from_string(template_str)
        template_sql = template.render(param_name=param)
        rendered = render_action(action)
        return template_sql == rendered
    else:
        raise ValueError(f"Unknown action type: {action.get('action')!r}")
