...
def main():
    ...
    source.add_argument("--collector-bundle", type=Path, help="Existing dbguard-collect.sh v0.2 output; unitless bool/enum settings only")
    parser.add_argument("--template-ref", type=Path, required=True)
    parser.add_argument("--spec-dir", type=Path, required=True)
    parser.add_argument("--records", type=Path, required=True)
    parser.add_argument("--assessment", type=Path, help="Optional upstream assessment; must match the exact normalized snapshot and specs")
