...
Legacy `/api/v1/sandbox/validate` and v0.2 snapshot intake are unchanged.
...
Re-ingesting a template at an existing version now clears its approval and
returns it to draft. Reviewers must approve the new content again; use a new
version if the previous approved version must remain available.

Upstream code can use the same collection/assessment adapter:

```python
snapshot = engine.collect(source_session)  # existing baseline SQL + exact spec queries
handoff = build_handoff(engine, snapshot, approved_template_reference)
payload = handoff.model_dump()
```

The source session's `sql(script)` method must support the existing collector's
psql script and provide its `target_id` variable. `SpecEngine` is currently the
local shared adapter; Xylon can replace it with the upstream implementation
while preserving this interface. The adapter evaluates all supplied specs,
including explicit manual findings. It does not generate specs from XLSX.
...
from accepting a request. This handoff is a proposed integration contract for
the team, not evidence that Xylon's full upstream pipeline has already migrated.
