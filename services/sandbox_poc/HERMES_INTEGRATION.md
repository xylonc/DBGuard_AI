...
The existing `POST /api/v1/snapshots` now stores Xylon's native v0.3 collector JSON.
Legacy uploads remain available, but sandbox use requires manifest collection.
See [INTEGRATION.md](INTEGRATION.md) for the 25 September Phase 1 connection.
...
1. **A PostgreSQL 17 collector snapshot**, produced by `collector/dbguard-collect.sh`
   with `-m checks.json` from a disposable/dev target with `log_connections=off`. It needs ungapped,
...
3. **The upstream assessment, if available**, bound to the same normalized snapshot
   and spec hashes. Native `assessment-report-v1` from `scripts/assess.py` is now
   accepted directly. The bridge computes it with the same assessor if omitted.
...
5. **For meaningful adaptive alternatives**, any separately reviewed template
   candidates that could correct a failed candidate, with their supporting evidence.
   Phase 1 permits only one active version per template name; archived versions
