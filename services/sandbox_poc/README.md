...
`SpecEngine.collect(session)` runs the existing `collector/collect.sql` and the
exact queries from the supplied specs. It fills in provenance for default-valued
spec settings omitted by the baseline. `SpecEngine.assess(snapshot)` uses the
existing comparison operators. Both source and sandbox call these same methods;
there is no parallel sandbox assessment rulebook.

This is a minimal adapter pending Xylon's upstream collector/spec-assessment
integration, not a replacement for his XLSX/spec-generation work. Snapshot API
intake remains v0.2.0. The local adapter emits the existing v0.3.0 JSON contract:
`envelope`, `baseline`, `checks`; `checks` hold collection results, not findings.
Assessment is a separate object with `snapshot_hash`, `spec_set_hash` and a
`findings` mapping keyed by `spec_id`. Exact equality with a fresh assessment
binds incoming assessment evidence to the snapshot and specs.
